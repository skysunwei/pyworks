...
WHERE paysuborder.orderid=payhistory.orderid
  ...
  AND FROM_unixtime(payhistory.paydateline,'%Y-%m-%d')='todaydatestr'
  AND paysuborder.ordersalerid IN (todaysalerids)
...
WHERE paysuborder.orderid=payhistory.orderid
  ...
  AND FROM_unixtime(paysuborder.revokedateline,'%Y-%m-%d')='todaydatestr'
  AND paysuborder.ordersalerid IN (todaysalerids)
GROUP BY paysuborder.orderid
...
WHERE vipuser_rebate.`recommend_user_id` IN (todaysalerids)
  AND FROM_UNIXTIME(vipuser_rebate.`confirm_time`, '%Y-%m-%d') = 'todaydatestr'
